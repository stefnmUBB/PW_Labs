...
async function SlidingPuzzle(div) {
	...
	self.host = div;	
	self.size = div.getAttribute("n");
	self.source = div.getAttribute("src");	
	...
	self.board = function(i, j) {						
		return self.table.children[i].children[j];
	}
	...
	for(var i=0;i<self.size;i++) {
		...
		for(var j=0;j<self.size;j++) {			
			...
			cell.setAttribute("pieceId",i*self.size+j);
			if(i!=self.size-1 || j!=self.size-1){
				if(self.source=="#") {
					cell.innerHTML=cell.pieceId;				
					...
					cell.style.background = `url('${self.source}') -${cw*j}px -${ch*i}px`;
				}					
			}						
	...
	self.host.innerHTML = "";
	self.host.appendChild(table);		
	
	function swap(node1, node2) {		
		var tmp = node1.cloneNode(true);
